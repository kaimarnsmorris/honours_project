...
% accent colours reused by the TikZ diagram and the Bayes-floor references
...
\begin{frame}{Bayesian inference is expensive --- and repeated}
  ...
  $\cdots$ \\[0.4em]
  \alert{the same expensive computation, paid again every time}
  \end{center}
\end{frame}
...
\begin{frame}{Neural Bayes estimators: pay once}
  ...
  \begin{tikzpicture}[>=Stealth, every node/.style={font=\small}]
    ...
    \node (net) [draw, fill=black!8, rounded corners, minimum height=14mm, minimum width=34mm, right=20mm of y] {network $\net$};
...
\begin{frame}{The observation this thesis starts from}
  ...
  \begin{itemize}
    ...
    \item That variance comes from \emph{how we sample the loss}, not from the inference problem. \alert{So we can reduce it.}
  \end{itemize}
\end{frame}
...
\begin{frame}{A trick for killing Monte Carlo noise}
  ...
  \vspace{0.6em}
  \begin{center}\small
  noisy draws \; $\rightarrow$ \; replace one coordinate by its exact mean \; $\rightarrow$ \; \alert{tighter estimate}
  \end{center}
  \vspace{0.4em}
...
\begin{frame}{The guarantee}
  ...
  \end{itemize}
  \vspace{0.4em}
  \begin{center}\alert{\textbf{For free, provably: never worse, usually better.}}\end{center}
\end{frame}
...
\begin{frame}{A contrasting lever: importance sampling}
  {\small No conjugacy needed --- reshape the simulation distribution, then reweight back. \alert{The price: no guarantee.}}
  \vspace{0.4em}
  \begin{columns}[c]
...
\begin{frame}{What separates the two techniques}
  ...
  \end{center}
  \vspace{0.8em}
  \begin{center}\alert{Where conjugacy exists, Rao--Blackwell is the safer, stronger lever.}\end{center}
\end{frame}
...
\begin{frame}{Takeaway}
  ...
  \end{itemize}
  \vspace{1.0em}
  \begin{center}\alert{\textbf{Conjugacy --- the classical trick behind Gibbs sampling ---\\
  is just as useful for training the estimators meant to replace it.}}\end{center}
\end{frame}
